...
function google_authUser() {
  ...
  if (isset($_SESSION['access_token']) && $_SESSION['access_token']) {
    ...
    if($query->get_result()->num_rows == 0) {
      $insert = $mysqli->prepare("INSERT INTO users (email, info) VALUES (?, ?)");
      $insert->bind_param("ss", $email, json_encode($userInfo));
      $insert->execute();
    }
    ...
    $lookup->setKeys([$key]);
    $result = $datasets->lookup($google_api_config['dataset-id'], $lookup);
    // User hasn't signed in before
    if(count($result->getFound()) == 0) {
